...
export function shimSource() {
    return `// safari-compat-shim.js — auto-generated by viaduct. Do not edit.
(function () {
  "use strict";
  ...
  // Safari has no native side panel. The converter injects sidepanel.html as a
  // docked iframe (sidepanel-inject.js content script) toggled by a message; here
  // we route chrome.sidePanel.* onto that so the extension's own "open panel"
  // calls toggle the injected panel instead of opening a detached tab.
  if (typeof chrome !== "undefined" && !chrome.sidePanel && chrome.tabs) {
    var c2sTogglePanel = function (msgType, tabId) {
      var send = function (id) {
        if (id == null) return;
        try { chrome.tabs.sendMessage(id, { type: msgType }, function () { void chrome.runtime.lastError; }); } catch (e) {}
      };
      if (tabId != null) { send(tabId); return Promise.resolve(); }
      try {
        var q = chrome.tabs.query({ active: true, currentWindow: true }, function (tabs) {
          send(tabs && tabs[0] && tabs[0].id);
        });
        if (q && q.then) q.then(function (tabs) { send(tabs && tabs[0] && tabs[0].id); });
      } catch (e) {}
      return Promise.resolve();
    };
    chrome.sidePanel = {
      open: function (opts) { return c2sTogglePanel("c2s-open-sidepanel", opts && opts.tabId); },
      // Chrome's "open the panel when the toolbar button is clicked" idiom: wire
      // action.onClicked to toggle the injected panel.
      setPanelBehavior: function (b) {
        if (b && b.openPanelOnActionClick && chrome.action && chrome.action.onClicked
            && !chrome.action.onClicked.__c2sPanelHooked) {
          chrome.action.onClicked.addListener(function (tab) {
            c2sTogglePanel("c2s-toggle-sidepanel", tab && tab.id);
          });
          chrome.action.onClicked.__c2sPanelHooked = true;
        }
        return Promise.resolve();
      },
  ...
  if (hasChrome && chrome.declarativeNetRequest) {
    ...
    // Safari's declarativeNetRequest omits the string-enum objects (RuleActionType,
    // HeaderOperation, ResourceType). The SW reads e.g.
    // declarativeNetRequest.RuleActionType.MODIFY_HEADERS at module-eval, which
    // null-derefs and aborts SW setup. Backfill the enums with their Chrome values.
    if (!dnrNs.RuleActionType) {
      dnrNs.RuleActionType = {
        BLOCK: "block", REDIRECT: "redirect", ALLOW: "allow",
        UPGRADE_SCHEME: "upgradeScheme", MODIFY_HEADERS: "modifyHeaders",
        ALLOW_ALL_REQUESTS: "allowAllRequests",
      };
    }
    if (!dnrNs.HeaderOperation) {
      dnrNs.HeaderOperation = { APPEND: "append", SET: "set", REMOVE: "remove" };
    }
    if (!dnrNs.ResourceType) {
      dnrNs.ResourceType = {
